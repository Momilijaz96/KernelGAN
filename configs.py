...
class Config:
    def __init__(self):
        ...
        self.parser.add_argument('--input_video_path', default=os.path.dirname(__file__) + '', help='path to one video file')
        self.parser.add_argument('--output_dir_path', default=os.path.dirname(__file__) + '', help='results path')
    ...
    def clean_file_name(self):
        ...
        self.conf.img_name = self.conf.input_video_path.split('/')[-1].replace('ZSSR', '') \
            .replace('real', '').replace('__', '').split('_.')[0].split('.')[0]
