...
def resize_tensor_w_kernel(im_t, k, sf=None):
    ...
    return F.conv3d(im_t, k, stride=(1,round(1 / sf),round(1 / sf)), padding=('same',padding,padding))
...
def create_gradient_map_video(vid, window=5, percent=.97):
    """Create a gradient map of the video blurred with a rect of size window and clips extreme values"""
    loss_maps = []
    for im in vid:
        # Calculate gradients
        gx, gy = np.gradient(rgb2gray(im))
        ...
        loss_map = convolve2d(grads_comb, np.ones(shape=(window, window)), 'same') / (window ** 2)
        # Normalizing: sum of map = numel
        loss_map = loss_map / np.mean(loss_map)
        #Loss maps gathered for each frame in a list
